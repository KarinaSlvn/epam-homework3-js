...
const sortCountRecallsIncrease = (firstItem, nextItem) => parseInt(firstItem.ratingRevievs) - parseInt(nextItem.ratingRevievs);
const sortCountRecallsDecrease = (firstItem, nextItem) => parseInt(nextItem.ratingRevievs) - parseInt(firstItem.ratingRevievs);

const sortCountPriceIncrease = (firstItem, nextItem) => {
    if(typeof(firstItem.price) === 'object' && typeof(nextItem.price) === 'string'){
       return parseInt(firstItem.price.newUan.replace(' ', '')) - parseInt(nextItem.price.replace(' ', ''))
    }
    else if(typeof(nextItem.price) === 'object'&& typeof(firstItem.price) === 'string'){
        return parseInt(firstItem.price.replace(' ', '')) - parseInt(nextItem.price.newUan.replace(' ', ''))
    }
    else if(typeof(firstItem.price) === 'object' && typeof(nextItem.price) === 'object'){
        return parseInt(firstItem.price.newUan.replace(' ', '')) - parseInt(nextItem.price.newUan.replace(' ', ''))
    }
    else{
        return parseInt(firstItem.price.replace(' ', '')) - parseInt(nextItem.price.replace(' ', ''))
    }
};
const sortCountPriceDecrease = (firstItem, nextItem) => {
    if(typeof(firstItem.price) === 'object' && typeof(nextItem.price) === 'string'){
        return parseInt(nextItem.price.replace(' ', '')) - parseInt(firstItem.price.newUan.replace(' ', ''))
    }
    else if(typeof(nextItem.price) === 'object'&& typeof(firstItem.price) === 'string'){
        return parseInt(nextItem.price.newUan.replace(' ', '')) - parseInt(firstItem.price.replace(' ', ''))
    }
    else if(typeof(firstItem.price) === 'object' && typeof(nextItem.price) === 'object'){
        return parseInt(nextItem.price.newUan.replace(' ', '')) - parseInt(firstItem.price.newUan.replace(' ', ''))
    }
    else{
        return parseInt(nextItem.price.replace(' ', '')) - parseInt(firstItem.price.replace(' ', ''))
    }
};
const getRatingRevievs = ()=>{
    let answerOnRatingRevievs = prompt(`Какой порядок сортировки отзывов вы хотите?
        1) От большего к меньшему - 1; 
        2) От меньшего к большему - 2;`);
    if (parseInt(answerOnRatingRevievs) === 1) console.log(arr.sort(sortCountRecallsDecrease));
    else if (parseInt(answerOnRatingRevievs) === 2) console.log(arr.sort(sortCountRecallsIncrease));
    else alert('Что то ввели не так :)');
}
const getPrice = ()=>{
    let answerOnPrice = prompt(`Какой порядок сортировки цены вы хотите?
        1) От большего к меньшему - 1; 
        2) От меньшего к большему - 2;`);
    if (parseInt(answerOnPrice) === 1) console.log(arr.sort(sortCountPriceDecrease));
    else if (parseInt(answerOnPrice) === 2) console.log(arr.sort(sortCountPriceIncrease));
    else alert('Что то ввели не так :)');
}
const callOrder = () => {
    let usersAnswer = prompt(`По какому параметру вы хотите отсортировать данные?
        1) Цена - 1; 
        2) Количество отзывов - 2;`);
    if (parseInt(usersAnswer) === 1) getPrice();
    else if (parseInt(usersAnswer) === 2) getRatingRevievs();
    else alert('Что то ввели не так :)');
};
window.onload = () => {
    callOrder();
